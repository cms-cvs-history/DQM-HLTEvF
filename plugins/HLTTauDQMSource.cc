...
HLTTauDQMSource::HLTTauDQMSource( const edm::ParameterSet& ps ) :counterEvt_(0)
{
  ...
  //Backup Path
  mainPath_               = mainParams.getUntrackedParameter<std::string>("MainFilter");
  l1BackupPath_           = mainParams.getUntrackedParameter<std::string>("L1BackupFilter");
  l2BackupPath_           = mainParams.getUntrackedParameter<std::string>("L2BackupFilter");
  l25BackupPath_          = mainParams.getUntrackedParameter<std::string>("L25BackupFilter");
  l3BackupPath_           = mainParams.getUntrackedParameter<std::string>("L3BackupFilter");

  refFilters_             = mainParams.getUntrackedParameter<std::vector<std::string> >("refFilters");
...
HLTTauDQMSource::beginJob(const EventSetup& context){
   ...
   if (dbe_) 
	 ...
	 {
	   dbe_->setCurrentFolder(mainFolder_);
	   triggerBitInfoRef_.push_back(dbe_->book1D((monitorName_+"_triggerBitInfo_"+refFilterDesc_[i]).c_str(),(monitorName_+":Trigger Bit (match to "+refFilterDesc_[i] + ") ").c_str(),8,0,10));
	   ...
	   formatHistogram(triggerBitInfoRef_[i],1);
   	   ...
   	   dbe_->setCurrentFolder(mainFolder_);
	   triggerEfficiencyRef_.push_back(dbe_->book1D((monitorName_+"_triggerEfficiencyRefTo_"+refFilterDesc_[i]).c_str(),("#tau Path Efficiency with ref to "+refFilterDesc_[i]).c_str() ,8,0,10));
...
HLTTauDQMSource::doL25(const Event& iEvent, const EventSetup& iSetup)
{
	  ...
	  if(iEvent.getByLabel(l25IsolInfo_, tauTagInfo))
	      ...
	      for(IsolatedTauTagInfoCollection::const_iterator tauTag  = tauTagInfo->begin();tauTag!=tauTagInfo->end();++tauTag)
		{
		  ...
		  const TrackRef lTrack =tauTag->leadingSignalTrack(matchCone_,minTrackPt_); 
		  if(!lTrack)
		    {
...
HLTTauDQMSource::doL3(const Event& iEvent, const EventSetup& iSetup)
{
	  ...
	  if(iEvent.getByLabel(l3IsolInfo_, tauTagInfo))
	      ...
	      for(IsolatedTauTagInfoCollection::const_iterator tauTag  = tauTagInfo->begin();tauTag!=tauTagInfo->end();++tauTag)
		{
		  ...
		  const TrackRef lTrack =tauTag->leadingSignalTrack(l3matchCone_,l3minTrackPt_); 
		  if(!lTrack)
		    {
...
LVColl 
HLTTauDQMSource::importFilterColl(std::string& filter,const Event& iEvent)
{
      //Create output Collection
      LVColl out;
